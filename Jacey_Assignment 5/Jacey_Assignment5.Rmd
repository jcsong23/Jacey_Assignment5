...
subject_acc_wide = subject_accuracy_wide %>%
  mutate(diff1 = phonological - semantic)

subject_acc_wide = subject_acc_wide %>% ungroup()

outliers = subject_acc_wide %>% identify_outliers(diff1)
...
hist(subject_acc_wide$diff1)
```
# t-test normality
```{r}
ggqqplot(subject_acc_wide, "diff1")

subject_acc_wide %>% shapiro_test(diff1)
